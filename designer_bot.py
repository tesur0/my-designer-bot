...
def compact_short_tz(text: str) -> str:
    ...
    compact = "\n".join(lines[:12])
    compact = re.sub(r"таблиц[аеиуой]* с планами[^,\n.]*", "таблица", compact, flags=re.IGNORECASE)
    if len(compact) > 950:
...
- строго не додумывай: если клиент сказал "в таблице", пиши "формат: таблица", а не "таблица с планами";
...
📦 Вводные:
1-2 коротких пункта: формат / текст / CTA / референс / продукт
